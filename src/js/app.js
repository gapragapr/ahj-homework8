...
nameFormBtn.onclick = (e) => {
    ...
    if (nameFormInpt.value !== ''){
        ...
        ws.send(JSON.stringify({method: "createNewUser", name: nameFormInpt.value}));
...
messageFormBtn.onclick = (e) => {
    ...
    if (messageFormInpt.value !== ''){
        ws.send(JSON.stringify({method: "createNewMessage", author: currentUser, text: messageFormInpt.value}))
    ...
    data.forEach(item => {
        ...
        <div class="message ${item.autor == currentUser ? 'my-message' : ''}">
            <div class="message-info">
                <span class="author">${item.autor == currentUser ? 'You' : item.autor}</span>
